...
async def upload_files_async(image_list: List[Image.Image], formatted_date: str, formatted_time: str) -> List[str]:
    start_time = time.time()
    ...
    async with session.client(
        ...
        for index, pil_image in enumerate(image_list):
            image_stream = BytesIO()
            pil_image.save(image_stream)
            image_stream.seek(0)
