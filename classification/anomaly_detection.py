...
# 현재 시각을 기반으로 하위 폴더명 생성
current_time = datetime.now().strftime('%Y%m%d%H%M%S')
# 출력 경로 설정 (현재 시각으로 하위 폴더 추가)
output_root = os.path.join('anomaly_maps', current_time)
# 출력 경로 디렉토리 생성
                ...
                if os.path.isfile(img_path):
                    try:
                        ...
                        with torch.no_grad():
                            ...
                            # 예측 확률 계산 (torch.softmax 또는 torch.sigmoid 사용)
...
def main():
    # 원하는 모델 이름을 설정
    model_name = "efficientnet_v2_s"  # 변경 가능: resnet34, efficientnet_v2_s, convnext_tiny, mobilenet_v3_small
    model = load_model(model_name)
    model.load_state_dict(torch.load(f'{model_name}_best2.pth', map_location=torch.device('cpu')), strict=False)
    # model.load_state_dict(torch.load(f'{model_name}_fine_tuned_20240924_021016.pth', map_location=torch.device('cpu')), strict=False)
    model.eval()
    ...
    # 선택된 모델로 이미지 처리
    process_images(model, device, output_root)
