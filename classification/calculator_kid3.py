...
class CustomDataset(Dataset):
    def __init__(self, dir1, dir2, transform=None):
        ...
        # 이미지 개수가 다를 경우 작은 쪽의 이미지를 반복 사용하기 위한 설정
        if len(self.dir1_images) != len(self.dir2_images):
            print(f"Warning: Unequal number of images in {dir1} and {dir2}. Using smaller dataset to repeat images.")

        self.max_len = max(len(self.dir1_images), len(self.dir2_images))  # 더 큰 쪽의 길이를 기준으로 설정

    def __len__(self):
        # 더 큰 쪽의 길이를 사용하여 반복적으로 비교할 수 있도록 설정
        return self.max_len

    def __getitem__(self, idx):
        # 이미지를 반복 사용하도록 작은 쪽의 인덱스를 나머지 연산을 통해 선택
        img1_path = self.dir1_images[idx % len(self.dir1_images)]
        img2_path = self.dir2_images[idx % len(self.dir2_images)]
