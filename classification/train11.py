...
# 모델 학습 및 평가 수행
# 기존 학습 함수에서 모델 불러오는 부분을 수정
def train_and_evaluate_models(model_names, train_loader, test_loaders_dict, categories, num_classes, learning_rate, num_epochs, output_dir, pretrained_paths=None):
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    results = []
...
base_model_dir = r"J:\conductzero\project\be\S11P21S001\ai\classification\results\20240927005345"
model_type = "full"
pretrained_paths = {
...
train_and_evaluate_models(model_names, train_loader, test_loaders_dict, categories, num_classes, learning_rate, num_epochs, output_dir, pretrained_paths)
