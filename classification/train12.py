...
class MultiCategoryGoodAndDefectDataset(Dataset):
    def __init__(self, root_dirs, transform=None):
        ...
        self.image_paths = []
        self.labels = []
        self.category_counts = {}  # 카테고리별 이미지 개수 저장

        for root_dir in root_dirs:
            category = os.path.basename(os.path.dirname(root_dir))
            self.category_counts[category] = {'good': 0, 'defect': 0}
            ...
            if os.path.exists(good_dir):
                ...
                self.category_counts[category]['good'] += len(good_images)  # good 이미지 개수 추가

            # defect 폴더 하위의 모든 이미지 로드
            ...
            if os.path.exists(defect_dir):
                for defect_subdir in os.listdir(defect_dir):
                    ...
                    if os.path.isdir(defect_subdir_path):
                        ...
                        self.category_counts[category]['defect'] += len(defect_images)  # defect 이미지 개수 추가
        ...
        for category, counts in self.category_counts.items():
            print(f"Category: {category} - Good Images: {counts['good']}, Defect Images: {counts['defect']}")
...
def save_evaluation_results(results, output_file, save_additional_format=False):
    ...
    # 추가 형식으로 저장 (카테고리별로 모델 비교)
    if save_additional_format:
        formatted_results = []

        # 카테고리별로 그룹화하여 모델별로 지표 저장
...
# 모델 학습 및 평가 수행
# 모델 학습 및 평가 수행
def train_and_evaluate_models_per_category(model_names, train_dirs, test_loaders_dict, categories, num_classes, learning_rate, num_epochs, output_dir):
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    results = []
    ...
    for model_name in model_names:
        ...
        for category, train_dir in zip(categories, train_dirs):
            print(f"--- Training for Category: {category} ---")

            # 각 카테고리의 학습 데이터 로더 생성
            train_transform = transforms.Compose([
                transforms.RandomResizedCrop(224),
                transforms.RandomHorizontalFlip(),
                transforms.ToTensor(),
            ...
            train_dataset = MultiCategoryGoodAndDefectDataset([train_dir], transform=train_transform)
            ...
            # 모델 저장 경로
            model_save_path = os.path.join(output_dir, f"{model_name}_best_model_{category}.pth")

            # 모델 학습
            model = train_model(model, dataloaders_dict, criterion, optimizer, num_epochs, model_save_path, save_dir=output_dir, model_name=f"{model_name}_{category}")

            # 전체 모델 저장
            full_model_path = os.path.join(output_dir, f"{model_name}_full_model_{category}.pth")
...
# 모델 학습 및 평가 수행
train_and_evaluate_models_per_category(model_names, train_dirs, test_loaders_dict, categories, num_classes, learning_rate, num_epochs, output_dir)