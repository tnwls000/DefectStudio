...
# 학습 시 MultiCategoryGoodAndDefectDataset 구현 (여러 카테고리의 good 및 defect 데이터 포함)
class MultiCategoryGoodAndDefectDataset(Dataset):
    def __init__(self, root_dirs, transform=None):
        """
        root_dirs: 각 카테고리별 train 폴더 경로 리스트 (각 경로에는 'good'과 'defect' 폴더가 존재)
        ...
        # 각 카테고리별 good과 defect 폴더의 이미지를 모두 로드
        for root_dir in root_dirs:
            ...
            if os.path.exists(good_dir):
                ...
                self.labels.extend([0] * len(good_images))  # good 이미지는 라벨 0
            ...
            if os.path.exists(defect_dir):
                for defect_subdir in os.listdir(defect_dir):
                    ...
                    if os.path.isdir(defect_subdir_path):
                        ...
                        self.labels.extend([1] * len(defect_images))  # defect 이미지는 라벨 1
...
# 테스트 시 CustomDefectDataset 구현 (good과 defect 모두 포함)
...
# 모든 카테고리의 good 및 defect 데이터 경로 리스트
...
# 학습 데이터 로더 (여러 카테고리의 good 및 defect 데이터 사용)
...
print("Full model saved as 'efficientnet_v2_s_full_model2.pth'")
