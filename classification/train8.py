...
def train_model(model, dataloaders, criterion, optimizer, num_epochs=20, model_save_path="best_model.pth"):
    ...
    # 학습 기록을 그래프로 시각화
    plot_training_history(train_losses, train_accuracies, num_epochs, save_path="training_history.png")

    return model


# 학습 기록 시각화 함수 정의
def plot_training_history(losses, accuracies, num_epochs, save_path=None):
    ...
    # 그래프를 이미지 파일로 저장
    if save_path:
        plt.savefig(save_path)  # 그래프를 이미지로 저장
        print(f"Training history saved as {save_path}")
    else:
        plt.show()  # 그래프를 화면에 출력

    plt.close()  # 그래프 창을 닫아 메모리 해제
...
# 모델 학습 및 평가 수행
def train_and_evaluate_models(model_names, train_loader, test_loaders_dict, categories, num_classes, learning_rate, num_epochs, output_file):
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    ...
    for model_name in model_names:
        ...
        model_save_path = f"{model_name}_{current_time}_best_model.pth"

        # 모델 학습
        model = train_model(model, dataloaders_dict, criterion, optimizer, num_epochs, model_save_path)

        # 전체 모델 저장
        torch.save(model, f"{model_name}_{current_time}_full_model.pth")
        print(f"Full model saved as '{model_name}_{current_time}_full_model.pth'")
...
# 평가 결과를 저장할 파일 경로
output_file = f"evaluation_results_{current_time}.csv"

# 모델 학습 및 평가
train_and_evaluate_models(model_names, train_loader, test_loaders_dict, categories, num_classes, learning_rate, num_epochs, output_file)
