...
const Signup = () => {
  ...
  return (
    <div className="w-full h-full min-h-[1024px] relative overflow-hidden bg-white">
      ...
      <form
        onSubmit={onSubmit}
        className="flex flex-col absolute top-[100px] left-1/2 transform -translate-x-1/2"
      >
        <input
          {...register("login_id", { required: "loginId is required" })}
          type="text"
          placeholder="loginId"
          className={`w-[90%] min-w-[400px] my-5 max-w-[400px] h-[50px] px-4 rounded-[10px] border ${
            errors.login_id ? "border-red-500" : "border-[#ccc]"
          } text-sm sm:text-base text-[#808080] focus:outline-none`}
        />
        {errors.login_id && (
          <p className="text-red-500 text-sm">{errors.login_id.message}</p>
        )}
