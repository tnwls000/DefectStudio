...
interface FabricContextProps {
  ...
  setActiveIndex: React.Dispatch<React.SetStateAction<{ lassoIndex: number; pointIndex: number }>>;
...
export const FabricProvider: React.FC<FabricProviderProps> = ({ children }) => {
  ...
  const [activeIndex, setActiveIndex] = useState<{ lassoIndex: number; pointIndex: number }>({
    lassoIndex: -1,
    pointIndex: -1
  });
  ...
  const [canvasDownloadUrl, setCanvasDownloadUrl] = useState<string | null>(null);
