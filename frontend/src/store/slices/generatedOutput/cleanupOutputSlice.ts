...
export interface CleanupOutputState {
  isLoading: boolean;
...
const initialState: CleanupOutputState = {
  isLoading: false,
  taskId: null,
  checkedOutput: true,
  output: {
...
const CleanupOutputSlice = createSlice({
  name: 'CleanupOutput',
  initialState,
  reducers: {
    // 이미지 생성 체크를 위한 로딩
...
export const {
  ...
  setselectedImgs,
  setMaxImgsNum
} = CleanupOutputSlice.actions;

export default CleanupOutputSlice.reducer;
