...
interface InpaintingOutputState {
  isLoading: boolean;
  taskId: string | null;
...
const initialState: InpaintingOutputState = {
  isLoading: false,
  taskId: null,
  checkedOutput: true,
  output: {
...
const inpaintingOutputSlice = createSlice({
  name: 'inpaintingOutput',
  initialState,
  reducers: {
    // 이미지 생성 체크를 위한 로딩
...
export const {
  ...
  setselectedImgs,
  setMaxImgsNum
} = inpaintingOutputSlice.actions;

export default inpaintingOutputSlice.reducer;
