...
export interface RemoveBgOutputState {
  isLoading: boolean;
...
const initialState: RemoveBgOutputState = {
  isLoading: false,
  taskId: null,
  checkedOutput: true,
  output: {
...
const removeBgOutputSlice = createSlice({
  name: 'RemoveBgOutput',
  initialState,
  reducers: {
    // 이미지 생성 체크를 위한 로딩
...
export const {
  ...
  setselectedImgs,
  setMaxImgsNum
} = removeBgOutputSlice.actions;

export default removeBgOutputSlice.reducer;
