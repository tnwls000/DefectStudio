...
export interface Txt2ImgOutputState {
  isLoading: boolean;
...
const initialState: Txt2ImgOutputState = {
  isLoading: false,
  taskId: null,
  checkedOutput: true,
  output: {
...
const txt2ImgOutputSlice = createSlice({
  name: 'txt2ImgOutput',
  initialState,
  reducers: {
    // 이미지 생성 체크를 위한 로딩
...
export const {
  ...
  setselectedImgs,
  setMaxImgsNum
} = txt2ImgOutputSlice.actions;

export default txt2ImgOutputSlice.reducer;
